...
mongoClient.connect(theApp.mongoDBConnection, function(err, db) {
    ...
    var createAppartementCollection = new Promise(function (resolve, reject) {
        // appartement collection
        db.createCollection("appartement", function (err, collection) {
            if (err) reject({success: false, err: err});

            console.log("Created appartement Collection");
    ...
    var createTicketColletion = new Promise(function (resolve, reject) {
        // ticket collection
    ...
    Promise.all([createAppartementCollection, createUserCollection, createTicketColletion])
